...
GameEngine::GameEngine(const std::string &userconfig, const std::string &default_config) {
...
void GameEngine::create_window(
	...
	if (fullscreen) {
		...
		std::cout << "created window: " << mode.width << "x" << mode.height << " (" << mode.bitsPerPixel << ")\n";
	} else {
...
void GameEngine::create_window(const sf::VideoMode & mode, const std::string & title, const size_t framerate, const bool fullscreen) {
